...
export async function POST(req: NextRequest) {
  ...
  const sheetRow = {
    ...
    Source: "getluckygolf.co.za /school-fundraising",
  };
  ...
  if (isDbConfigured()) {
    try {
      ...
      console.error("School lead DB write failed", err);
    }
  }

  const tasks = await Promise.allSettled([
    appendSubmission("school", sheetRow),
    sendSubmissionNotification("school", sheetRow),
  ]);

  const failures = tasks.filter((t) => t.status === "rejected");
  if (failures.length === tasks.length) {
    console.error("School submission both failed", failures);
    return NextResponse.json(
  ...
  if (failures.length > 0) {
    console.warn("School submission partial failure", failures);
  }
