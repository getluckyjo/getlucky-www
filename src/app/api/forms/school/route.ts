...
export async function POST(req: NextRequest) {
  ...
  const emailed = await sendSubmissionNotification("school", submission).then(
    () => true,
    (err) => {
      console.error("School notification email failed", err);
      return false;
    },
  );
  ...
  if (!recordedInDb && !emailed) {
    console.error("School submission not recorded and not emailed");
    return NextResponse.json(
