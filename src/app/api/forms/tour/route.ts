...
export async function POST(req: NextRequest) {
  ...
  const sheetRow = {
    ...
    "Tours Per Year": data.toursPerYear || "",
    Message: data.message || "",
    Source: "getluckygolf.co.za /golf-tours",
  };
  ...
  if (isDbConfigured()) {
    try {
      await insertLead({
        ...
        data: { tours_per_year: data.toursPerYear || "" },
      });
    } catch (err) {
      console.error("Tour operator lead DB write failed", err);
    }
  }

  const tasks = await Promise.allSettled([
    appendSubmission("tour", sheetRow),
    sendSubmissionNotification("tour", sheetRow),
  ]);

  const failures = tasks.filter((t) => t.status === "rejected");
  if (failures.length === tasks.length) {
    console.error("Tour operator submission both failed", failures);
    return NextResponse.json(
  ...
  if (failures.length > 0) {
    console.warn("Tour operator submission partial failure", failures);
  }
