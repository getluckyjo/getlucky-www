...
export async function POST(req: NextRequest) {
  ...
  const emailed = await sendSubmissionNotification("tour", submission).then(
    () => true,
    (err) => {
      console.error("Tour operator notification email failed", err);
      return false;
    },
  );
  ...
  if (!recordedInDb && !emailed) {
    console.error("Tour operator submission not recorded and not emailed");
    return NextResponse.json(
