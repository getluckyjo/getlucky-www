...
export default function CharityForm() {
  ...
  useEffect(() => {
    if (!messageTouched && quote) {
      setMessage(formatCharityQuoteMessage(quote));
    }
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      charityName: String(fd.get("charityName") || ""),
    ...
    try {
      const res = await fetch("/api/forms/charity", {
        method: "POST",
  ...
  if (submitted) {
    return (
      <FormSuccessCard
        title="Charity day enquiry received"
        body="Thanks — a Get Lucky activation specialist will be in touch within 24 hours to set up your charity golf day and confirm the next steps."
      />
    );
  }
  ...
  return (
    <form onSubmit={onSubmit} noValidate className="space-y-5">
      ...
      <div className="grid sm:grid-cols-2 gap-5">
        <Field label="Charity / Organisation Name" name="charityName" required error={errors.charityName}>
          <Input name="charityName" required autoComplete="organization" />
        </Field>
      ...
      >
        <Textarea
          name="message"
          rows={10}
          placeholder="Tell us about your day — your cause, expected number of golfers, and what you'd like the fundraiser to achieve."
          value={message}
          onChange={(e) => {
